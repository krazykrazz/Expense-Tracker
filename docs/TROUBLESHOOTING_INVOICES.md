...
3. **Split Large Files:**
   ...
   - Upload each page as separate expense if needed
...
#### Issue: "Expense already has an invoice" Error

**Symptoms:**
- Error message: "This expense already has an invoice attached"
- HTTP 409 Conflict response
- Cannot upload new invoice

**Causes:**
- Expense already has an invoice attached
- Trying to upload instead of replace
- Previous upload succeeded but UI didn't update

**Solutions:**

1. **Replace Existing Invoice:**
   - Edit the expense
   - Click "Replace" button (not "Upload")
   - Select new file
   - Save expense

2. **Delete Then Upload:**
   - Edit the expense
   - Click "Delete" to remove existing invoice
   - Save expense
   - Edit again and upload new invoice

3. **Refresh Page:**
   - Reload the page
   - Check if invoice already exists
   - Use replace function if needed

**Prevention:**
- Check for existing invoice before uploading
- Use "Replace" function for updates
- Verify upload success before retrying
...
3. **Delete Expense:**
   - Deleting expense removes invoice
...
| "Invoice not found" | 404 | No invoice attached | Upload invoice first |
| "This expense already has an invoice" | 409 | Duplicate upload | Use replace function |
