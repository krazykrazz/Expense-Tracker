...
/**
 * Migration: Mark existing auto-generated billing cycles as reviewed
 * 
 ...
 * surfaces all cycles where is_user_entered = 0 AND actual_statement_balance = 0.
 * This causes pre-existing cycles to show up as "new" notifications.
 * 
 * This migration marks all existing auto-generated cycles as user-entered (reviewed)
 * so the banner starts clean and only shows genuinely new auto-generated cycles.
 */
async function migrateMarkExistingAutoGeneratedCyclesAsReviewed(db) {
  const migrationName = 'mark_existing_auto_generated_cycles_reviewed_v1';
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION', (err) => {
        ...
        // Mark all existing unreviewed auto-generated cycles as reviewed
        db.run(
          `UPDATE credit_card_billing_cycles 
           SET is_user_entered = 1 
           WHERE is_user_entered = 0 AND actual_statement_balance = 0`,
          function(err) {
            if (err) {
              db.run('ROLLBACK');
              return reject(err);
            ...
            const updatedCount = this.changes;
            logger.info(`Marked ${updatedCount} pre-existing auto-generated billing cycles as reviewed`);
