...
class BillingCycleRepository {
      ...
      return new Promise((resolve, reject) => {
        ...
        db.run(sql, params, function(err) {
          ...
          resolve({
            ...
            effective_balance: data.effective_balance !== undefined ? data.effective_balance : null,
            balance_type: data.balance_type || 'calculated'
          });
        });
      });
    }
  ...
  async update(id, data) {
    ...
    return new Promise((resolve, reject) => {
      // First get the existing record to preserve unchanged fields
      db.get('SELECT * FROM credit_card_billing_cycles WHERE id = ?', [id], (err, existing) => {
        ...
        const params = [
          ...
          data.effective_balance !== undefined ? data.effective_balance : existing.effective_balance,
          data.balance_type !== undefined ? data.balance_type : (existing.balance_type || 'calculated'),
          id
        ];
  ...
  async updateCalculatedBalance(id, calculatedBalance, effectiveBalanceData) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const params = [
        calculatedBalance,
        effectiveBalanceData && effectiveBalanceData.effective_balance !== undefined
          ? effectiveBalanceData.effective_balance
          : null,
        effectiveBalanceData && effectiveBalanceData.balance_type
          ? effectiveBalanceData.balance_type
          : 'calculated',
        id
      ];
