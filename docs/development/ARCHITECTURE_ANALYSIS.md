...
### 4. ~~Group frontend components by domain~~ ✅ COMPLETED

**Priority: MEDIUM | Effort: MEDIUM | Risk: LOW**

**Status:** Completed via frontend-component-domain-grouping spec. The flat `frontend/src/components/` directory (~80 JSX + ~65 CSS files) was reorganized into 9 domain-based subdirectories: `expenses/`, `financial/`, `credit-cards/`, `loans/`, `tax/`, `analytics/`, `notifications/`, `system/`, `shared/`. Each domain has an `index.js` barrel export. All cross-domain imports, `vi.mock()` paths, CSS `@import` paths, and filesystem paths in tests were updated. Zero behavioral changes — refactor only.
