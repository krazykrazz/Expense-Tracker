...
- [ ] 1. Set up database schema and migration
  ...
  - Add loans table with columns: id, name, rate, initial_balance, start_date, notes, is_paid_off, created_at, updated_at
  - Add loan_balances table with columns: id, loan_id, year, month, remaining_balance, created_at, updated_at
...
- [ ] 2. Implement loan repository layer
  - [ ] 2.1 Create loanRepository.js with CRUD operations
  ...
  - [ ] 2.2 Create loanBalanceRepository.js with balance operations
...
- [ ] 3. Implement loan service layer
  - [ ] 3.1 Create loanService.js with business logic
    - Implement validateLoan(loan) method for name, rate, initial_balance, start_date validation
    ...
    - Implement markPaidOff(id, isPaidOff) method
    - Implement getAllLoans() method returning loans with current balances
  ...
  - [ ] 3.2 Create loanBalanceService.js with balance business logic
    - Implement validateBalanceEntry(entry) method for year, month, remaining_balance validation
    - Implement createOrUpdateBalance(data) method with upsert logic
    - Implement updateBalance(id, data) method
    - Implement deleteBalance(id) method
    - Implement getBalanceHistory(loanId) method with change calculations
...
- [ ] 4. Implement loan controller layer
  - [ ] 4.1 Create loanController.js with HTTP handlers
  ...
  - [ ] 4.2 Create loanBalanceController.js with balance HTTP handlers
...
- [ ] 5. Create API routes
  ...
  - [ ] 5.2 Create loanBalanceRoutes.js
...
- [ ] 7. Create frontend API service
  ...
  - [ ] 7.2 Create loanBalanceApi.js service
  ...
  - [ ] 7.3 Update config.js with new API endpoints
    - Add LOANS endpoint constant
    - Add LOAN_BALANCES endpoint constant
    - _Requirements: 1.1, 2.1_

- [ ] 8. Enhance SummaryPanel component to display loans
...
- [ ] 9. Create LoansModal component
  ...
  - [ ] 9.2 Add loan form functionality to LoansModal
    - Create add/edit loan form with fields: name, rate, initial_balance, start_date, notes
    - Implement form validation (required fields, rate >= 0, initial_balance >= 0)
...
- [ ] 10. Create LoanDetailView component
  - [ ] 10.1 Create LoanDetailView.jsx structure
    ...
    - Display loan summary card with original amount, current balance, total paid down, rate, start date
  ...
  - [ ] 10.2 Add balance history timeline to LoanDetailView
    - Display balance history table with columns: Month/Year, Remaining Balance, Change, Actions
    - Sort balance entries chronologically (most recent first)
    - Calculate change from previous month for each entry
    - Apply color coding: green for decreases (negative change), red for increases (positive change)
    - Add edit and delete buttons for each balance entry
    - Implement handleEditBalance(id) to show inline edit form
    - Implement handleDeleteBalance(id) with confirmation using loanBalanceApi.deleteBalance()
    - _Requirements: 4.1, 4.2, 4.4, 4.5_

  - [ ] 10.3 Add balance entry form to LoanDetailView
    - Create add balance entry form with month/year picker and remaining_balance input
    - Implement form validation (month 1-12, year valid, balance >= 0)
    - Implement handleAddBalance() using loanBalanceApi.createOrUpdateBalance()
    - Show message if balance entry already exists for selected month (will update)
    - Refresh balance history after add/update
    - Auto-scroll to newly added entry
    - _Requirements: 2.1, 2.2, 2.3_

  - [ ] 10.4 Add loan actions to LoanDetailView
  ...
  - [ ] 10.5 Create LoanDetailView.css
    - Style modal overlay and container
...
- [ ] 12. Add database backup integration
...
- [ ] 13. Update API endpoint configuration
