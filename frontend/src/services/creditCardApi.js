...
/**
 * Get unified billing cycles with auto-generation, transaction counts, and trend indicators
 * @param {number} paymentMethodId - Payment method ID (must be credit_card type with billing_cycle_day)
 * @param {Object} [options] - Query options
 ...
 * @returns {Promise<Object>} Unified billing cycles response
 * {
 *   success: boolean,
 *   billingCycles: Array<{
 *     id: number,
 ...
 *     trend_indicator: { type: string, icon: string, amount: number, cssClass: string } | null,
 ...
 * }
 */
export const getUnifiedBillingCycles = async (paymentMethodId, options = {}) => {
  try {
    const params = new URLSearchParams();
    if (options.limit !== undefined) {
      params.append('limit', options.limit.toString());
    }
    if (options.includeAutoGenerate !== undefined) {
      params.append('include_auto_generate', options.includeAutoGenerate.toString());
    }
