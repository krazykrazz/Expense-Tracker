...
/**
 * @invariant Trend data generation must cover the correct time range with gap filling for missing
 * months, and month-over-month change percentages must be correctly calculated.
 * 
 * Randomness adds value by testing trend calculation correctness across varying expense patterns,
 * date gaps, and spending amounts that would be impractical to enumerate manually.
 * 
...
describe('MerchantAnalyticsService - Trend Data Generation Property Tests', () => {
  ...
  describe('Property 8: Trend data covers correct time range with gap filling', () => {
    test('Trend data generation with gap filling', async () => {
      await fc.assert(
        fc.asyncProperty(
  ...
  describe('Property 9: Month-over-month change percentage is correctly calculated', () => {
    test('Month-over-month change calculation', async () => {
      await fc.assert(
        fc.asyncProperty(
          // Generate merchant name
          fc.string({ minLength: 1, maxLength: 20 }).filter(s => s.trim().length > 0),
