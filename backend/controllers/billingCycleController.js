 ...
 * GET /api/payment-methods/:id/billing-cycles/recalculate
 * 
 * Query params:
 * - startDate (required): string YYYY-MM-DD, cycle start date
 * - endDate (required): string YYYY-MM-DD, cycle end date
 * 
 * Returns:
 * - calculatedBalance: number
 */
async function recalculateBalance(req, res) {
  try {
    ...
    if (!endDate || !dateRegex.test(endDate)) {
      return res.status(400).json({
        success: false,
        error: 'endDate is required in YYYY-MM-DD format'
      });
    }
    
    const calculatedBalance = await billingCycleHistoryService.recalculateBalance(
      paymentMethodId,
      startDate,
      endDate
    );
 ...
 * Called when the user dismisses the auto-generated cycle notification banner.
 * POST /api/billing-cycles/dismiss-auto-generated
 */
async function dismissAutoGeneratedNotifications(req, res) {
  try {
    const count = await billingCycleRepository.markAllAutoGeneratedAsReviewed();
    logger.info('Dismissed auto-generated cycle notifications:', { reviewedCount: count });
    res.status(200).json({ reviewedCount: count });
  } catch (error) {
    logger.error('Failed to dismiss auto-generated cycle notifications:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
}
...
module.exports = {
  ...
  getCreditCardDetail,
  recalculateBalance,
  dismissAutoGeneratedNotifications
};
