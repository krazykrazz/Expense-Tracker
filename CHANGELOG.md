...
### Added
- Background billing cycle scheduler that automatically creates billing cycle records when cycles end
- Configurable cron schedule via `BILLING_CYCLE_CRON` environment variable (default: daily at 2:00 AM)
- Auto-generation notification banner alerting users to review calculated balances
- Activity logging for all scheduler operations (cycle creation, run summaries, errors)
- Initial startup check (60 seconds after boot) to catch missed cycles during downtime

### Changed
- CreditCardDetailView no longer triggers lazy auto-generation (now handled by background scheduler)
- Reminder status API response includes new `autoGeneratedCycleNotifications` field
