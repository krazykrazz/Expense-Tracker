...
/**
 * Migration: Add statement_pdf_path column to credit_card_billing_cycles table
 * Allows attaching PDF statements to billing cycle records
 */
async function migrateAddStatementPdfToBillingCycles(db) {
  const migrationName = 'add_statement_pdf_to_billing_cycles_v1';
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION', (err) => {
        ...
        // Add statement_pdf_path column to credit_card_billing_cycles table
        db.run(`
        ...
        `, (err) => {
          if (err) {
            ...
            if (err.message && err.message.includes('duplicate column name')) {
              logger.info('Column statement_pdf_path already exists, skipping');
            } else {
              db.run('ROLLBACK');
              return reject(err);
            }
          ...
          logger.info('Added statement_pdf_path column to credit_card_billing_cycles table');
...
/**
 * Migration: Migrate PDF statements from credit_card_statements to billing_cycle_history
 * This consolidates the old PDF statement uploads into the new billing cycle history system
 * 
 * For each PDF statement:
 * 1. Creates a billing_cycle_history record with the statement's period dates
 * 2. Copies the PDF file path to statement_pdf_path
 * 3. Sets actual_statement_balance to 0 (user can update later)
 * 4. Deletes the old credit_card_statements record
 */
async function migratePdfStatementsToBillingCycles(db) {
  const migrationName = 'migrate_pdf_statements_to_billing_cycles_v1';
  ...
  return new Promise((resolve, reject) => {
    // First, get all existing PDF statements
    db.all(`
      SELECT 
        ccs.id,
        ccs.payment_method_id,
        ccs.statement_date,
        ccs.statement_period_start,
        ccs.statement_period_end,
        ccs.file_path,
        ccs.original_filename,
        pm.billing_cycle_day
      FROM credit_card_statements ccs
      JOIN payment_methods pm ON pm.id = ccs.payment_method_id
      ORDER BY ccs.statement_period_end ASC
    `, [], (err, statements) => {
      if (err) {
        logger.error('Failed to fetch PDF statements for migration:', err);
        return reject(err);
      }

      if (!statements || statements.length === 0) {
        logger.info('No PDF statements to migrate');
        // Mark migration as applied even if no statements
        markMigrationApplied(db, migrationName).then(() => {
          logger.info(`Migration "${migrationName}" completed (no statements to migrate)`);
          resolve();
        }).catch(reject);
        return;
      }

      logger.info(`Found ${statements.length} PDF statements to migrate`);
      ...
      db.serialize(() => {
        db.run('BEGIN TRANSACTION', (err) => {
          ...
          const processStatement = (index) => {
            if (errorOccurred || index >= statements.length) {
              // All done, commit or rollback
              if (errorOccurred) {
                db.run('ROLLBACK');
                return;
              }

              // Delete migrated statements from old table
              db.run('DELETE FROM credit_card_statements WHERE id IN (SELECT id FROM credit_card_statements)', (err) => {
                if (err) {
                  logger.warn('Failed to delete old statements (non-critical):', err.message);
                }
                ...
                markMigrationApplied(db, migrationName).then(() => {
                  db.run('COMMIT', (err) => {
                    ...
                    logger.info(`Migration "${migrationName}" completed: ${migratedCount} migrated, ${skippedCount} skipped`);
                    resolve();
                  });
                }).catch((err) => {
                  db.run('ROLLBACK');
            ...
            // Check if a billing cycle record already exists for this period
            db.get(`
              SELECT id FROM credit_card_billing_cycles 
              WHERE payment_method_id = ? AND cycle_end_date = ?
            `, [stmt.payment_method_id, stmt.statement_period_end], (err, existing) => {
              if (err) {
                logger.error('Failed to check existing billing cycle:', err);
                errorOccurred = true;
                db.run('ROLLBACK');
                return reject(err);
              }
              ...
              if (existing) {
                // Update existing record with PDF path if it doesn't have one
                db.run(`
                  UPDATE credit_card_billing_cycles 
                  SET statement_pdf_path = COALESCE(statement_pdf_path, ?)
                  WHERE id = ? AND statement_pdf_path IS NULL
                `, [stmt.file_path, existing.id], (err) => {
                  if (err) {
                    logger.warn('Failed to update existing billing cycle with PDF:', err.message);
                  } else {
                    logger.debug(`Updated existing billing cycle ${existing.id} with PDF path`);
                  }
                  skippedCount++;
                ...
                // First calculate the statement balance from expenses in the billing period
                db.get(`
                  ...
                  WHERE payment_method_id = ?
                    ...
                    AND COALESCE(posted_date, date) <= ?
                `, [stmt.payment_method_id, stmt.statement_period_start, stmt.statement_period_end], (err, row) => {
                  if (err) {
                    logger.error('Failed to calculate statement balance:', err);
                    errorOccurred = true;
                    db.run('ROLLBACK');
                    return reject(err);
                  }

                  const calculatedBalance = Math.round((row?.total || 0) * 100) / 100;
                  ...
                  `, [
                    ...
                    if (err) {
                      logger.error('Failed to create billing cycle from PDF statement:', err);
                      errorOccurred = true;
                      db.run('ROLLBACK');
                      return reject(err);
                    }
                    
                    logger.debug(`Created billing cycle ${this.lastID} from PDF statement ${stmt.id} with calculated balance ${calculatedBalance}`);
                    migratedCount++;
                    processStatement(index + 1);
                  });
                });
...
/**
 * Migration: Add reviewed_at column and mark existing auto-generated cycles as reviewed
 * 
 * Before v5.11.0, auto-generated billing cycles existed but there was no
 * notification banner. v5.11.0 added the AutoGeneratedCycleBanner which
 * surfaces unreviewed auto-generated cycles.
 * 
 * This migration adds a reviewed_at timestamp column to track when a user
 * has acknowledged an auto-generated cycle, and marks all pre-existing
 * auto-generated cycles as reviewed so the banner starts clean.
 * 
 * This replaces the original approach of flipping is_user_entered = 1,
 * which corrupted the balance display logic (calculateEffectiveBalance
 * uses is_user_entered to decide between actual vs calculated balance).
 */
async function migrateMarkExistingAutoGeneratedCyclesAsReviewed(db) {
  const migrationName = 'add_reviewed_at_to_billing_cycles_v2';
  ...
  // Check if reviewed_at column already exists
  ...
  const hasReviewedAt = columns.some(col => col.name === 'reviewed_at');
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION', (err) => {
        ...
        const afterColumnAdded = () => {
          // Mark all existing auto-generated cycles as reviewed
          // (cycles that existed before the notification banner was added)
          db.run(
            `UPDATE credit_card_billing_cycles 
            ...
            function(err) {
              ...
              const updatedCount = this.changes;
              logger.info(`Marked ${updatedCount} pre-existing auto-generated billing cycles as reviewed (via reviewed_at)`);
        ...
        if (!hasReviewedAt) {
          db.run(
            'ALTER TABLE credit_card_billing_cycles ADD COLUMN reviewed_at TEXT DEFAULT NULL',
            (err) => {
              if (err) {
                db.run('ROLLBACK');
                return reject(err);
              }
              logger.info('Added reviewed_at column to credit_card_billing_cycles');
              afterColumnAdded();
            }
          );
        } else {
...
/**
 * Migration: Rename 'Vehicle Maintenance' category to 'Automotive'
 */
async function migrateRenameVehicleMaintenanceToAutomotive(db) {
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION', (err) => {
        ...
        const tables = [
          { table: 'expenses', column: 'type' },
          { table: 'fixed_expenses', column: 'category' },
          { table: 'budgets', column: 'category' },
          { table: 'place_names', column: 'category' }
        ];

        let completed = 0;
        let hadError = false;

        function onTableDone(err, tableName) {
          if (hadError) return;
          if (err) {
            hadError = true;
            db.run('ROLLBACK');
        ...
        tables.forEach(({ table, column }) => {
          ...
          db.get(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`, [table], (err, row) => {
            if (err) return onTableDone(err, table);
            if (!row) {
              logger.info(`Table "${table}" does not exist, skipping`);
              return onTableDone(null, table);
            }
            db.run(
              `UPDATE ${table} SET ${column} = 'Automotive' WHERE ${column} = 'Vehicle Maintenance'`,
              function(err) {
                if (!err) {
                  logger.info(`Updated ${this.changes} rows in ${table}`);
                }
                onTableDone(err, table);
              }
            );
          });
        });
  ...
  try {
    if (hasStaleExpensesConstraint) {
      ...
      try {
        ...
        // Copy all data, renaming 'Vehicle Maintenance' → 'Automotive' during the copy
        // This avoids the CHECK constraint violation that would occur with a separate UPDATE
        await runStatement(db, `
          ...
          SELECT id, date, posted_date, place, notes, amount,
            CASE WHEN type = 'Vehicle Maintenance' THEN 'Automotive' ELSE type END,
            week, method, payment_method_id, insurance_eligible, claim_status, original_cost, created_at
          FROM expenses
        `);
    ...
    if (hasStaleBudgetsConstraint) {
      ...
      try {
        ...
        // Rename 'Vehicle Maintenance' → 'Automotive' during the copy
        await runStatement(db, `
          INSERT INTO budgets_new (id, year, month, category, "limit", created_at, updated_at)
          SELECT id, year, month,
            CASE WHEN category = 'Vehicle Maintenance' THEN 'Automotive' ELSE category END,
            "limit", created_at, updated_at
          FROM budgets
        `);
    ...
    // Also update fixed_expenses and place_names which don't have CHECK constraints
    // but may still have stale 'Vehicle Maintenance' data
    await runStatement(db, `UPDATE fixed_expenses SET category = 'Automotive' WHERE category = 'Vehicle Maintenance'`).catch(() => {});
    await runStatement(db, `UPDATE place_names SET category = 'Automotive' WHERE category = 'Vehicle Maintenance'`).catch(() => {});
