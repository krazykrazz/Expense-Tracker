...
describe('PlaceNameService - Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    await new Promise((resolve, reject) => {
      db.run("DELETE FROM expenses WHERE place LIKE 'INTEG_%'", (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
  ...
  describe('standardizePlaceNames', () => {
    test('should standardize place names with transaction', async () => {
      // Insert test data
      await insertTestExpense('INTEG_Walmart');
      await insertTestExpense('INTEG_walmart');
      await insertTestExpense('INTEG_Wal-Mart');

      const updates = [
        { from: ['INTEG_Walmart', 'INTEG_walmart', 'INTEG_Wal-Mart'], to: 'INTEG_Walmart_Final' }
      ];
      ...
      expect(result.message).toBe('Successfully standardized 3 expense records');

      // Verify the update
      const finalCount = await countExpensesByPlace('INTEG_Walmart_Final');
      expect(finalCount).toBe(3);
    });

    test('should reject invalid updates', async () => {
      const invalidUpdates = [
        { from: ['test'], to: '' } // Empty 'to' field
      ];

      await expect(placeNameService.standardizePlaceNames(invalidUpdates))
        .rejects.toThrow("'to' cannot be empty or whitespace");
    });

    test('should handle multiple groups in single transaction', async () => {
      // Insert test data
      await insertTestExpense('INTEG_Store1');
      await insertTestExpense('INTEG_store1');
      await insertTestExpense('INTEG_Store2');
      await insertTestExpense('INTEG_store2');

      const updates = [
        { from: ['INTEG_Store1', 'INTEG_store1'], to: 'INTEG_Store1_Standard' },
        { from: ['INTEG_Store2', 'INTEG_store2'], to: 'INTEG_Store2_Standard' }
      ];
      ...
      const store1Count = await countExpensesByPlace('INTEG_Store1_Standard');
      const store2Count = await countExpensesByPlace('INTEG_Store2_Standard');
      
      expect(store1Count).toBe(2);
      expect(store2Count).toBe(2);
    });
  });
});
