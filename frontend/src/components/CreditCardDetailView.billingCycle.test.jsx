...
describe('CreditCardDetailView - Billing Cycle Integration', () => {
  ...
  const buildUnifiedResponse = (overrides = {}) => ({
    cardDetails: overrides.cardDetails || mockCreditCardWithBillingCycle,
    payments: overrides.payments || [],
    statementBalanceInfo: overrides.statementBalanceInfo !== undefined ? overrides.statementBalanceInfo : mockStatementBalanceCalculated,
    currentCycleStatus: overrides.currentCycleStatus !== undefined ? overrides.currentCycleStatus : mockCycleStatusNoActual,
  ...
  describe('Section Visibility Based on billing_cycle_day', () => {
    it('should show billing cycles tab when billing_cycle_day is configured', async () => {
      const enrichedCycles = mockBillingCycleHistory.billingCycles.map(c => ({
        ...c,
        effective_balance: c.actual_statement_balance || c.calculated_statement_balance,
        balance_type: c.actual_statement_balance > 0 ? 'actual' : 'calculated',
        transaction_count: 5,
        trend_indicator: null
      }));
      creditCardApi.getCreditCardDetail.mockResolvedValue(buildUnifiedResponse({
        currentCycleStatus: mockCycleStatusWithActual,
      ...
      // Should show the Billing Cycles tab
      await waitFor(() => {
        expect(screen.getByRole('button', { name: /Billing Cycles/i })).toBeInTheDocument();
      });
    });

    it('should show Billing Cycles tab with 0 count when billing_cycle_day is NOT configured', async () => {
      creditCardApi.getCreditCardDetail.mockResolvedValue(buildUnifiedResponse({
        cardDetails: mockCreditCardWithoutBillingCycle,
      ...
      // The Billing Cycles tab is now always visible (shows 0 count when no billing_cycle_day)
      expect(screen.getByRole('button', { name: /Billing Cycles/i })).toBeInTheDocument();
    });
    ...
    it('should show "Enter Statement" button when billing_cycle_day is configured', async () => {
      ...
      // Should show the Enter Statement button in the billing cycles list
      await waitFor(() => {
    ...
    it('should NOT show "Enter Statement" button when billing_cycle_day is NOT configured', async () => {
      ...
      // Should NOT show the Enter Statement button (no billing cycles)
      await waitFor(() => {
  ...
  describe('Property 9: Statement Balance Display Priority', () => {
    ...
    it('should display calculated statement balance when no billing cycle record exists', async () => {
      creditCardApi.getCreditCardDetail.mockResolvedValue(buildUnifiedResponse({
        currentCycleStatus: mockCycleStatusNoActual
      }));
        ...
        fc.asyncProperty(
          ...
          async (actualBalance, calculatedBalance) => {
            ...
            creditCardApi.getCreditCardDetail.mockResolvedValue(buildUnifiedResponse({
              ...
              currentCycleStatus: cycleStatusWithActual
            }));
  ...
  describe('Billing Cycle History Tab', () => {
    it('should display billing cycles in the Billing Cycles tab', async () => {
      const enrichedCycles = mockBillingCycleHistory.billingCycles.map(c => ({
        ...c,
        effective_balance: c.actual_statement_balance || c.calculated_statement_balance,
        balance_type: c.actual_statement_balance > 0 ? 'actual' : 'calculated',
      ...
      // Click on Billing Cycles tab
      const billingTab = screen.getByRole('button', { name: /Billing Cycles/i });
      await act(async () => {
        fireEvent.click(billingTab);
      });

      // Should show billing cycles content (the UnifiedBillingCycleList component)
      await waitFor(() => {
        // The unified list shows cycle dates - check for the unified-cycle-dates class
        const cycleDates = document.querySelector('.unified-cycle-dates');
        expect(cycleDates).toBeTruthy();
        expect(cycleDates.textContent).toContain('Jan 26, 2026');
      });
    });
  });
